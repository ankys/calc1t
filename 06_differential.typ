...
#block[
$x -> a$で
$ frac(frac(f(x), g(x))-frac(f(a), g(a)), x-a) = frac(f(x) g(a)-f(a) g(x), g(x) g(a) (x-a)) = frac((f(x)-f(a)) g(a)-f(a) (g(x)-g(a)), g(x) g(a) (x-a)) -> frac(f' (a) g(a)-f(a) g' (a), g(a)^2) $
より示される。
...
#block[
$x -> a$で$f(x) -> f(a)$に注意して、
$ frac(g(f(x))-g(f(a)), x-a) = frac(g(f(x))-g(f(a)), f(x)-f(a)) frac(f(x)-f(a), x-a) -> g' (f(a)) f' (a) $
より示される。
...
- \$x \\in X\\setminus\\lrset{a}\$で定義された関数$frac(f(x)-f(a), x-a)$が右からの極限$x -> a +$で収束する時、
  ...
  などで表す。 つまり、
  $ f'_(+) (a) = lim_(x -> a +) frac(f(x)-f(a), x-a) = lim_(h -> 0 +) frac(f(a+h)-f(a), h) $
  である。

- \$x \\in X\\setminus\\lrset{a}\$で定義された関数$frac(f(x)-f(a), x-a)$が左からの極限$x -> a -$で収束する時、
...
実数の集合$X$で$X$のすべての点が$X$の内点になっているものを_開集合_という。
開区間は開集合である。
...
== 種々の関数の微分
<種々の関数の微分>
$c$を実数として定数関数$c$は$RR$で微分可能で、 \$\$\\dv{x}(c)
...
$0$でない実数$x$と$n = 1, 2, 3, dots$に対して負べき関数$x^(- n)$は\$\\mathbb{R}\\setminus\\lrset{0}\$で微分可能で、
\$\$\\begin{aligned}
\\dv{x}(x^{-n})
&= \\lim\_{h \\to 0}\\frac{(x+h)^{-n}-x^{-n}}{h}
...
= \\cos x.\$\$ 余弦関数$cos$も$RR$で微分可能で、 \$\$\\dv{x}(\\cos x)
...
= -\\frac{1}{\\sqrt{1-x^2}}.\$\$
逆正接関数$arctan$は$RR$で微分可能で、 \$\$\\dv{x}(\\arctan x)
...
双曲線正弦関数$sinh x$は$RR$で微分可能で、 \$\$\\dv{x}(\\sinh x)
= \\dv{x}\\qty(\\frac{e^x-e^{-x}}{2})
= \\frac{e^x+e^{-x}}{2}
= \\cosh x.\$\$ 双曲線余弦関数$cosh x$も$RR$で微分可能で、
\$\$\\dv{x}(\\cosh x)
= \\dv{x}\\qty(\\frac{e^x+e^{-x}}{2})
= \\frac{e^x-e^{-x}}{2}
= \\sinh x.\$\$ 双曲線正接関数$tanh x$は$RR$で微分可能で、
\$\$\\dv{x}(\\tanh x)
...
床関数\$\\floor\$と天井関数\$\\ceil\$はともに$RR \\ ZZ$で微分可能で、
\$\$\\dv{x}(\\floor(x))
= \\dv{x}(\\ceil(x))
= 0.\$\$ $x in ZZ$では微分可能でないことに注意する。
しかしながら、\$\\floor\$は$x in ZZ$で右微分可能であり、\$\\ceil\$は$x in ZZ$で左微分可能である。
...
- $X$の内点$a$であって、ある正の数$delta > 0$が存在して$f(a) = max_(B_delta (a)) f$となる$a in X$のことを$f$の_極大点_といい、$f$は点$a$で極大、またこの時の$f(a)$を$f$の_極大値_という。
  つまり、$a$を$f$の極大点とするとある正の数$delta > 0$が存在して任意の$x in B_delta (a)$つまり$abs(x-a) < delta$を満たす点$x in X$に対して$f(x) <= f(a)$が成り立つ。

- $X$の内点$a$であって、ある正の数$delta > 0$が存在して$f(a) = min_(B_delta (a)) f$となる$a in X$のことを$f$の_極小点_といい、$f$は点$a$で極小、またこの時の$f(a)$を$f$の_極小値_という。
  つまり、$a$を$f$の極小点とするとある正の数$delta > 0$が存在して任意の$x in B_delta (a)$つまり$abs(x-a) < delta$を満たす点$x in X$に対して$f(x) >= f(a)$が成り立つ。
...
この時
$ frac(f(x)-f(a), x-a) <= 0 quad (a < x < a+delta), quad frac(f(x)-f(a), x-a) >= 0 quad (a-delta < x < a) . $
したがって、 $ f'_(+) (a) <= 0, quad f'_(-) (a) >= 0 $
であり、$a$で微分可能なので命題@t_semidiffより、
$ f' (a) = f'_(+) (a) = f'_(-) (a) = 0 $ である。
最大点の時は極大点なので同じ証明でよい。
最小点、極小点の時は同様の議論をすることで#link(<e_maxp_semi>)[\[e\_maxp\_semi\]_の代わりに
$ f'_(+) (a) >= 0, quad f'_(-) (a) <= 0 $
が得られて同じ結論が導かれる。

]
#block[
証明を見ればわかる通り微分可能でなくとも、関数$f$が極大点$a$で右微分可能ないし左微分可能とすると不等式#link(<e_maxp_semi>)[\[e\_maxp\_semi\]_が成り立ち、関数$f$が極小点$a$で右微分可能ないし左微分可能とすると不等式#link(<e_minp_semi>)[\[e\_minp\_semi\]_が成り立つ。

]
この定理が重要な理由は関数$f$の最大点・最小点を見つける問題が方程式$f' (x) = 0$を解くことに帰着されることにある。

#block[
<t_gauss_est> $RR$上の連続関数 $ f(x) = x e^(- x) $ を考える。
この関数は$f(1) = e^(- 1) > 0$, $lim_(x -> -oo) f(x) = -oo$,
$lim_(x -> +oo) = 0$なので、最大値を持つ。
...
$ lim_(x -> a +) frac(f(x)-f(a), x-a) = f'_(+) (a) < 0 . $
...
- $f$がすべての内点$x$で$f' (x) >= 0$を満たすならば、$f$は広義単調増加である。

- $f$がすべての内点$x$で$f' (x) < 0$を満たすならば、$f$は狭義単調減少である。

- $f$がすべての内点$x$で$f' (x) <= 0$を満たすならば、$f$は広義単調減少である。
...
ここで、$x -> a$で$f(x) -> 0$,
$g(x) -> 0$であり、$frac(f' (x), g' (x))$が収束するならば、$frac(f(x), g(x))$も収束して
...
である。
このような$x$に対してこのような$c$の中から一つ選んで$c (x)$と定めると$a < c (x) < x$なので、$x -> a +$とすると$c (x) -> a$となることと$lim_(x -> a) frac(f' (x), g' (x))$が存在することから、
$ lim_(x -> a +) frac(f(x), g(x)) = lim_(x -> a +) frac(f' (c (x)), g' (c (x))) = lim_(x -> a +) frac(f' (x), g' (x)) . $
...
$g$は\$I\\setminus\\lrset{a}\$上の連続関数で\$I\\setminus\\lrset{a}\$上で微分可能であり各\$x \\in I\\setminus\\lrset{a}\$に対して$g' (x) eq.not 0$とする。
ここで、$x -> a$で$f(x) -> +oo$,
$g(x) -> +oo$であり、$frac(f' (x), g' (x))$が収束するならば、$frac(f(x), g(x))$も収束して
$ lim_(x -> a) frac(f(x), g(x)) = lim_(x -> a) frac(f' (x), g' (x)) $
が成り立つ。
...
$epsilon > 0$を固定すると、$delta > 0$が存在して任意の$a < x <= a+delta$に対して\$\\abs{\\frac{f\'(x)}{g\'(x)}-l} \< \\varepsilon\$である。
...
が成り立つ。
ここで、先に$x -> a +$とすることを考えると、$f(x) -> +oo$,
$g(x) -> +oo$であることから、極限が存在すれば
$ lim_(x -> a) frac(f(y), g(y)) = lim_(x -> a +) frac(f' (c (x, y)), g' (c (x, y))) . $
次いで、$y -> a +$とすれば$c -> a +$より、
$ lim_(x -> a) frac(f(y), g(y)) = lim_(x -> a +) frac(f' (x), g' (x)) . $
...
$x, y in I$を取ると、中間値の定理より$abs(f(x)-f(y)) = abs(f ' (c)) abs(x-y)$となる$c$が存在するので、$abs(f(x)-f(y)) <= L abs(x-y)$である。

]
#block[
このではこのことをもとに$n = 1, 2, 3, dots$と$x, y in RR$に対して不等式
$ abs(sin^n x-sin^n y) <= abs(x^n-y^n) $ が成立することを示す。
...
$ f(z) <= frac(z-x, y-x) (f(y)-f(x))+f(x) = frac(y-z, y-x) f(x)+frac(z-x, y-x) f(y) $
と同値で、$p = frac(y-z, y-x)$とおくと凸関数の条件式$f(p x+(1-p) y) <= p f(x)+(1-p) f(y)$に他ならない。
もう一つの不等号は
$ f(z) <=-frac(y-z, y-x) (f(y)-f(x))+f(y) = frac(y-z, y-x) f(x)+frac(z-x, y-x) f(y) $
と同値で同様に成り立つ。

2.ならば1.について、$0 < p < 1$と$x < y$を満たす$x, y in I$に対して#link(<e_convex>)[\[e\_convex\]_を示せばよい。
$z = p x+(1-p) y$とおくと、
$ f(p x+(1-p) y) = f(z) <= frac(y-z, y-x) f(x)+frac(z-x, y-x) f(y) = p f(x)+(1-p) f(y) $
である。
...
3.ならば2.について、1.ならば2.のところでわかるように$frac(f(z)-f(x), z-x) <= frac(f(y)-f(x), y-x)$と$frac(f(y)-f(x), y-x) <= frac(f(y)-f(z), y-z)$は同値で、片方が成り立たないとするともう片方も成り立たず、
$ frac(f(z)-f(x), z-x) > frac(f(y)-f(x), y-x) > frac(f(y)-f(z), y-z) $
となる。
...
- 対数関数$f(x) = log x$は$f'' (x) =-1 / x^2 < 0$なので、$RR$上の凹関数である。
...
- $f$が凸関数のとき、\$x \\in I\\setminus\\lrset{a}\$に対する関数$frac(f(x)-f(a), x-a)$は単調増加である。
  つまり$x <= y$を満たす任意の\$x, y \\in I\\setminus\\lrset{a}\$に対して、
  $ frac(f(x)-f(a), x-a) <= frac(f(y)-f(a), y-a) $
  が成り立つ。
...
- $x <= y < a$の場合、特徴づけより$frac(f(x)-f(a), x-a) = frac(f(a)-f(x), a-x) <= frac(f(a)-f(y), a-y) = frac(f(y)-f(a), y-a)$。

- $x < a < y$の場合、特徴づけより$frac(f(x)-f(a), x-a) = frac(f(a)-f(x), a-x) <= frac(f(y)-f(a), y-a)$。

- $a < x <= y$の場合、特徴づけより$frac(f(x)-f(a), x-a) <= frac(f(y)-f(a), y-a)$。
...
#block[
凸関数の方について示す。
前の命題より任意の$x < a < y$に対して$frac(f(x)-f(a), x-a) <= frac(f(y)-f(a), y-a)$が成り立つ。
ここで命題@t_supinf_indepより
$ frac(f(x)-f(a), x-a) <= k <= frac(f(y)-f(a), y-a) $
となる実数$k$が存在する。
...
#block[
$f$を区間$I$上の実数値関数とする。
$N = 1, 2, 3, dots$個の$a_1, dots, a_N$を$I$上の点、$0 <= p_1, dots, p_N <= 1$を$p_1+dots+p_N$を満たす数とする。
...
#block[
凸関数の方について示す。
命題@t_convex_suppより$f(x) >= f(a)+k (x-a)$を満たす実数$k$が存在する。
ここで$a = p_1 a_1+dots+p_N a_N$、$x = a_n$
($n = 1, dots N$)として$p_n$倍して足し合わせると、$p_1+dots+p_N = 1$に注意して、
$ sum_(n = 1)^N p_n f(a_n) >= sum_(n = 1)^N p_n (f(a)+k (a_n-a)) = f(a)+k (sum_(n = 1)^N p_n a_n-a) = f(a) . $
よってほしかった不等式を得る。
...
$ log(frac(a_1+dots+a_N, N)) >= 1 / N (log a_1+dots+log a_N) = log(root(N, a_1 dots a_N)) $
よって、
$ frac(a_1+dots+a_N, N) >= root(N, a_1 dots a_N) $
を得る。
この左辺は相加平均（算術平均）、右辺は相乗平均（幾何平均）と呼ばれ、今回の不等式は正の数$a_1, dots, a_N$に対して相乗平均より相加平均の方が常に大きいことを示していて相加平均・相乗平均の関係式と呼ばれる。
