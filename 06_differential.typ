...
$
を_ライプニッツの公式_という。
また、より$0$に近い$x = 1/sqrt(3)$の場合を考えると収束が早くより効率よく計算でき、_シャープの方法_という。
以下ではこのシャープの方法での円周率の評価を行う。
...
$
が成り立つ。
ここで、$-27/64 <= (5 c^4-10 c^2+1)/(1+c^2)^5 <= 1$がわかるので、
$
x-1/3 x^3-1/5 27/64 x^5 < arctan x < x-1/3 x^3+1/5 x^5
quad (0 <= x <= 1)
$
となる。
ここで$x = 1/sqrt(3)$とすると$arctan(1/sqrt(3)) = pi/6$であるので、
$
2533/1440 sqrt(3) = 3.0467254830360875 dots
< pi
< 82/45 sqrt(3) = 3.156181471569954 dots
$
が保証される。
]
