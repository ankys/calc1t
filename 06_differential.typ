...
= lim_(h -> 0) (binom(n, 0)x^n+binom(n, 1)h x^(n-1)+binom(n, 2)h^2 x^(n-2)+cdots+binom(n, n)h^n-x^n)/h
= lim_(h -> 0) (n x^(n-1)+binom(n, 2)h x^(n-2)+cdots+binom(n, n)h^(n-1))
= n x^(n-1)
$
である。
...
= lim_(h -> 0) (-n x^(n-1)-binom(n, 2)h x^(n-2)+cdots+binom(n, n)h^(n-1))/((x+h)^n x^n) \
&= (-n x^(n-1))/(x^(2 n))
= -n x^(-n-1)
$
である。
...
$
dv(, x)(a_0+a_1 x+a_2 x^2+a_3 x^3+cdots+a_n x^n)
...
quad dv(, x)(x^(-n)) = -n x^(-n-1) quad (n = 1, 2, 3, dots, x ne 0).
$
$
dv(, x)(a_0+a_1 x+a_2 x^2+a_3 x^3+cdots+a_n x^n)
= a_1+2 a_2 x+3 a_3 x^2+cdots+n a_n x^(n-1)
...
$
$
dv(, x)(x abs(x)^(a-1)) = a abs(x)^(a-1) quad (a in RR, x ne 0).
$
$
...
$
$
dv(, x)(log abs(x)) = 1/x quad (x ne 0),
quad dv(, x)(log_a abs(x)) = 1/((log a)x) quad (a > 0, a eq.not 1, x ne 0).
$
$
dv(, x)(sin x) = cos x,
