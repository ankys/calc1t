...
$k = 0, 1, 2, 3, dots$に対して微分を繰り返すことで$k$階微分を定義することができる。

#definition([高階微分])[
$f$を開集合$X$上で定義された実数値関数で$k = 0, 1, 2, 3, dots$とする。
ここで以下の帰納的に定義される関数$f^((k))$が存在したら$f$の_$k$階微分導関数_という。
- $f^((0)) = f$とする。
- $f^((k+1))$は$f^((k))$の微分導関数である。
...
$
などで表す。
$k$階微分導関数が存在する時は$f$は_$k$階微分可能_であるという。
すべての$k = 0, 1, 2, 3, dots$に対して$k$階微分可能である時は$f$は_無限階微分可能_であるという。
]
...
$
である。
$f^((n)) (x) = n!$となり定数関数なので、$k > n$のときは$f^((k)) = 0$である。
そのため、べき関数$f(x) = x^n$は無限階微分可能であり、後の用語で表現すると$C^oo (RR)$の元である。
]

#example[
指数関数$f(x) = exp(x) = e^x$を考えると、微分しても不変なので、$k = 0, 1, 2, 3, dots$に対して
$
f^((k)) (x) = f(x) = e^x
$
である。
...
今後の議論で重要になるのは$k$階微分可能であり$k$階微分導関数が連続であるものである。

#definition([連続的微分])[
$f$を開集合$X$上で定義された実数値関数であり、$k = 0, 1, 2, 3, dots$とする。
ここで$f$は$k$階微分可能であり$k$階微分導関数$f^((k))$が$X$上の連続関数であるとき、$f$は$X$上_$k$回連続的微分可能_または$X$上の_$C^k$級関数_であるといい、そのような関数の集合を$C^k (X)$と表す。
すべての$k = 0, 1, 2, 3, dots$に対して$k$回連続的微分可能であるとき、$f$は_無限回連続的微分可能_または$X$上の_$C^oo$級関数_であるといい、そのような関数の集合を$C^oo (X)$と表す。
]

#remark[
定義から$k$回連続的微分可能ならば$k$階微分可能である。
連続関数でないと微分可能ではないので、$k+1$階微分可能ならば$k$回連続的微分可能である。
そのため
$
C^0 (X) supset C^1 (X) supset C^2 (X) supset C^3 (X) supset dots supset C^k (X) supset C^oo (X)
$
が成り立ち、$C^0 (X)$は$X$上の連続関数全体の集合である。
特に$k$回連続的微分可能ならば$k$階までの微分導関数はすべて連続関数である。
