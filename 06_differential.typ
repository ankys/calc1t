...
このとき、度合い関数$omega_(f, a, N)$が存在して
$
f(x) = P_N (x)+omega_(f, a, N) (abs(x-a)) (x-a)^N
$
が成り立つ。
]

この時の$R_N (x) = omega_(f, a, N) (abs(x-a)) (x-a)^N$を_ペアノの剰余項_という。
...
$
が成り立つ。
よって、$omega_(f, a, N) (r) = sup_(abs(x-a) <= r) abs(f(x)-P_N (x))/abs(x-a)^N$とすればよい。
]
