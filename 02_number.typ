...
つまり、次の条件を満たす有理数の集合$A subset bb(Q)$を_有理数の切断_という。
...
_実数_はこのような有理数の切断であり、1つの有理数を取り除いたら有理数の切断ではなくなるものである。
...
実数$a = A$,
$b = B$の大小関係を$A subset B$を満たすとき$a <= b$、そうでない時$a > b$として定義する。
...
+ （推移性）任意の実数$a, b, c$に対して$a <= b$かつ$b <= c$ならば$a <= c$。

実数は有理数の隙間を埋めて得られると考えられるが、有理数は2つの実数の間にぎっしり詰まっていて次が成立する。

#block[
<t_dense_rational> 実数$a$,
$b$が$a < b$を満たす時、$a < x < b$を満たす有理数$x$が存在する。

]
#proof[
有理数の切断による。
$a$の有理数の切断としての表現を$A$、$b$の有理数の切断としての表現を$B$とする。
$overline(A)$を$a$が有理数のとき\$A\\cup\\lrset{ a }\$、そうでないとき$A$として定義すると$overline(A)$も有理数の切断で、仮定より$x in B \\ overline(A)$となる有理数$x$が存在する。
...
+ （乗法の結合法則）任意の実数$a, b, c$に対して$(a dot b) dot c = a dot (b dot c)$。

+ （乗法の交換法則）任意の実数$a, b$に対して$a dot b = b dot a$。

+ （分配法則）任意の実数$a, b, c$に対して$a dot (b+c) = a dot b+a dot c$、$(a+b) dot c = a dot c+b dot c$。

+ （零元）$0$は零元である。つまり任意の実数$a$に対して$a+0 = 0+a = a$と$a dot 0 = 0 dot a = 0$が成り立つ。
...
+ （大小関係の加法との両立）任意の実数$a, b, c$に対して$a <= b$ならば$a+c <= b+c$。

+ （大小関係の乗法との両立）任意の実数$a, b, c$に対して$a <= b$,
  $c >= 0$ならば$a c <= b c$。

== 拡大実数
<拡大実数>
実数に_正の無限大_$+ oo$と_負の無限大_$- oo$を入れた数を_拡大実数_という。
有理数の切断で説明するならば、負の無限大は空集合$nothing$が正の無限大は全体集合$bb(Q)$がそれぞれ対応する。
_拡大実数全体の集合_を$bb(R)^(‾)$で表す。 つまり、
...
$ - oo <= a <= + oo $ が成り立ち、以下の性質が成り立つ。

+ （全順序）任意の拡大実数$a, b$に対して$a <= b$または$a >= b$のどちらかが成立し、両方が成立する場合は$a = b$に他ならない。

+ （推移性）任意の拡大実数$a, b, c$に対して$a <= b$かつ$b <= c$ならば$a <= c$。
...
特に$bb(R)^(‾) = [- oo, + oo]$で$bb(R) = (- oo, + oo)$である。

拡大実数の四則演算は実数のものに以下を付け加える。
つまり$a$を実数として、
$ (+ oo) + a = a + (+ oo) = + oo, quad (+ oo) + (+ oo) = + oo, $
$ (- oo) + a = a + (- oo) = - oo, quad (- oo) + (- oo) = - oo . $
拡張実数の反数は$- (+ oo) = - oo$, $- (- oo) = + oo$とすればよい。
$a$を正の実数として、
$ (+ oo) dot a = a dot (+ oo) = + oo, quad (+ oo) dot (- a) = (- a) dot (+ oo) = - oo, quad (+ oo) dot (+ oo) = + oo, quad (+ oo) dot (- oo) = - oo, $
$ (- oo) dot a = a dot (- oo) = - oo, quad (- oo) dot (- a) = (- a) dot (- oo) = + oo, quad (- oo) dot (+ oo) = - oo, quad (- oo) dot (- oo) = + oo, $
...
複素数$z = a+b i eq.not 0$の逆数は$z^(-1) = frac(a, a^2+b^2) - frac(b, a^2+b^2) i$であり、除法は次になる。
...
+ （分配法則）任意の複素数$a, b, c$に対して$a dot (b+c) = a dot b+a dot c$、$(a+b) dot c = a dot c+b dot c$。

+ （零元）$0$は零元である。つまり任意の複素数$a$に対して$a+0 = 0+a = a$と$a dot 0 = 0 dot a = 0$が成り立つ。

+ （反数）任意の複素数$a$に対して$a+x = x+a = 0$が成り立つような複素数$x = - a$がただ一つ存在する。

+ （単位元）$1$は単位元である。つまり任意の複素数$a$に対して$a dot 1 = 1 dot a = a$が成り立つ。
...
なお、この部分の内容は後の内容を使うが自然数の大小関係が定義された段階で最大元・最小元（定義@d_max）、有界性（定義@d_bdd）、命題@t_maxdiscが得られるので問題ない。

まず、自然数$n$と$0$でない自然数$m$を考え、自然数の集合\$A = \\lrset{ x \\in \\mathbb{N} \\mid x m \\le n }\$を定義する。
$m >= 1$なので、この時任意の$x in A$に対して$x <= x m <= n$より$A$は上に有界で命題@t_maxdiscから$A$の最大元$q$が存在する。
...
剰余付き除法の重要な点は$q$, $r$は$n = q m+r$,
$r < m$を満たす自然数であるということで、一意に存在する。
...
以上により整数$a$を正の整数$b$で割ったときの剰余付き除法の商$q$と剰余$r$が定義され、どの場合でも$q$,
$r$は$a = q b+r$ ($0 <= r < b$)を満たす整数である。

有理数での剰余付き除法を定義するために次の命題を準備する。

#block[
<t_int_rational> 有理数$a$に対して、$x <= a <= y$を満たす整数$x$,
$y$が存在する。

]
#proof[
有理数$a$は整数$n$と正の整数$m$を使って$a = n\/m$と表現できる。
ここで$n$を$m$で割った剰余付き除法の商を$q$、剰余を$r$とすると$q$,
$r$は$n = q m+r$,
$0 <= r < m$を満たす整数なので、$q m <= n < q m+m$つまり$q <= a < q+1$である。
よって命題が示された。~◻
...
#block[
<t_int_real> 実数$a$に対して、$x <= a <= y$を満たす整数$x$,
$y$が存在する。
...
$y'$が存在する。
さらに命題@t_int_rationalより$x <= x' < a < y' <= y$を満たす整数$x$,
$y$が存在する。 よって命題が示された。
]
...
実数$a$に対して命題@t_int_realから$a$以下の整数の集合に最大元が存在しそれを$a$の_床_といい\$\\lrfloor{a}\$と表す。
つまり、
\$\$\\lrfloor{a} = \\max\\lrset{ n \\in \\mathbb{Z} \\mid n \\le a }\$\$
である。
$a$の床\$\\lrfloor{a}\$は$a$を$1$で剰余付き除法をした時の商に他ならない。
$a$の床\$\\lrfloor{a} \\in \\mathbb{Z}\$を$a$の_整数部分_とも言い、残りの部分\$a-\\lrfloor{a} \\in \[0, 1)\$を$a$の_小数部分_という。

同様にして、実数$a$に対して命題@t_int_realから$a$以上の整数の集合に最小元が存在しそれを$a$の_天井_といい\$\\lrceil{a}\$と表す。
つまり、
...
- $c eq.not 1$の時、$sum_(n = M)^N c^n = frac(c^(N+1) - c^M, c -1)$。
...
$M$,
$N$を$M <= N$を満たす自然数として$N - M+1$個の数$a_M, dots, a_N$の積について考える。
$1$に$a_M, dots, a_N$を順番にかけて得られる数を$a_M, dots, a_N$の_総乗_といい、$product_(n = M)^N a_n$と表す。
つまり、 $ product_(n = M)^N a_n = a_M times dots times a_N $
である。 実際には乗法の結合法則と交換法則よりかける順番は関係ない。
...
== 二項定理
<二項定理>
自然数$n$, $k$を$k <= n$をみたすつまり$n - k$が自然数となるとする。
...
$ (a+b)^n = sum_(k = 0)^n binom(n, k) a^k b^(n - k) = binom(n, 0) b^n+binom(n, 1) a b^(n -1)+dots+binom(n, n -1) a^(n -1) b+binom(n, n) a^n . $
...
$ frac(a^2+b^2, 2) >= a b . $ 等号成立条件は$a = b$である。

]
#block[
平方根はまだ定義されていないが、$a$,
$b$が平方根の場合を考えると、任意の$a >= 0$, $b >= 0$に対して、
$ frac(a+b, 2) >= sqrt(a b) $ が成立する。
ここで、$frac(a+b, 2)$を$a$,
$b$の_算術平均_または_相加平均_、$sqrt(a b)$を$a$,
$b$の_幾何平均_または_相乗平均_という。
そのため今回の不等式は算術平均は幾何平均以上であることを保証し、_算術平均・幾何平均の不等式_という。

]
#proof[
$ frac(a^2+b^2, 2) - a b = 1 / 2 (a^2+b^2 - 2 a b) = 1 / 2 (a - b)^2 >= 0 . $
]
...
#block[
任意の実数$a_1, dots, a_N$,
$b_1, dots, b_N$に対して次が成立する。
\$\$\\qty(\\sum\_{n = 1}^N a\_n b\_n)^2 \\le \\qty(\\sum\_{n = 1}^N a\_n^2)\\qty(\\sum\_{n = 1}^N b\_n^2).\$\$
...
#proof[
任意の実数$t$に対して
\$\$\\sum\_{n = 1}^N (a\_n t+b\_n)^2 = \\qty(\\sum\_{n = 1}^N a\_n^2)t^2+2\\qty(\\sum\_{n = 1}^N a\_n b\_n)t+\\qty(\\sum\_{n = 1}^N a\_n^2) \\ge 0\$\$
に注意する。
