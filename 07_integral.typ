...
本テキストでは一般のベルヌーイ多項式の詳細は省略し、$N = 0, 1$の場合を示す。
...
2. $f$を$I$上の$C^2$級関数とすると、
  $
  integral_a^b f(x) dd(x)
  = &(f(a)+f(b))/2 (b-a)-1/12 (f'(b)-f'(a)) (b-a)^2 \
    &quad +1/2 integral_a^b [(x-(a+b)/2)^2-1/12 (b-a)^2] f''(x) dd(x)
  $
  が成り立つ。
]
...
$
-1/12 (b-a)^2 <= (x-(a+b)/2)^2-1/12 (b-a)^2 <= 1/6 (b-a)^2
$
である。
]
...
二つ目の主張は
$
&integral_a^b [(x-(a+b)/2)^2-1/12 (b-a)^2] f'' (x) dd(x) \
&quad = eval([((x-(a+b)/2)^2-1/12 (b-a)^2) f' (x)])_a^b-2 integral_a^b (x-(a+b)/2) f'(x) dd(x) \
&quad = 1/6 (f'(b)-f'(a))(b-a)^2-2 integral_a^b (x-(a+b)/2) f'(x) dd(x)
$
より一つ目に帰着される。
]
...
#example([階乗の漸近近似])[
階乗をオイラー・マクローリンの和公式で評価する。
つまり$f(x) = log x$、$M = 1$、$N = n$とする。
出現する項は順番に
$
integral_1^n log x dd(x) = n log n-n+1,
quad (log 1+log n)/2 = 1/2 log n,
quad (b_2)/(2!) [f' (n)-f' (1)] = 1/12 (1/n-1),
quad dots
$
で、$K = 2$として打ち止めすると
$
R_3 = integral_1^n (B_3 (x-floor(x)))/(3!) 2/x^3 dd(x),
quad abs(R_3) <= max_([0, 1]) B_3/3 (1-1/n^2)
$
である。
したがって
$
abs(log n!-(n log n-n+1+1/2 log n+1/12 (1/n-1))) <= max_([0, 1]) B_3/3 (1-1/n^2)
$
であり、
ここから$log n!$の漸近近似として$n log n-n+1/2 log n$が得られて、
階乗に直すと$n!$と$sqrt(n) (n/e)^n$でその比は
$
exp(11/12+1/(12 n)-max_([0, 1]) B_3/3 (1-1/n^2))
<= (n!)/(sqrt(n) (n/e)^n) <=
exp(11/12+1/(12 n)+max_([0, 1]) B_3/3 (1-1/n^2))
$
を満たし、有界な範囲に留まるので、
