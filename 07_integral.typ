...
つまり$x in [a, b]$に対して
$
f'((a+b)/2) (x-(a+b)/2)+f((a+b)/2) <= f(x) <= (f(b)-f(a))/(b-a) (x-a)+f(a)
$
が成り立つ。
...
#theorem[
$N = 0, 1, 2, 3, dots$として$I$を開区間とし、$f$を$I$上の$C^(N+1)$級関数、$B_(N+1) (x)$を$N+1$次のベルヌーイ多項式、$b_n$をベルヌーイ数とする。
...
#example([階乗の漸近近似])[
そのため階乗の漸近近似を得るためにはさらに評価を精密化する必要がある。
ここでは$log k = integral_1^k 1/x dd(x)$に注意して、
$
sum_(k = 1)^n log k
= sum_(k = 1)^n integral_1^k 1/x dd(x)
= sum_(k = 1)^n sum_(m = 1)^(k-1) integral_m^(m+1) 1/x dd(x)
= sum_(m = 1)^(n-1) (n-m) integral_m^(m+1) 1/x dd(x)
$
という変形をもとに考える。
とりあえず
$
log n! = sum_(k = 1)^n log k = n log n-sum_(m = 1)^(n-1) integral_m^(m+1) m/x dd(x)
$
である。
ここで$1/x$は$(0, oo)$上の凸関数なので、
$
(2 m)/(m+(m+1)) = 1-1/(2 m+1) <= integral_m^(m+1) m/x dd(x) <= 1/2 (m/m+m/(m+1)) = 1-1/(2 m+2)
$
より、
$
n log n-n+1+1/2 sum_(m = 1)^(n-1) 1/(m+1)
<= log n!
<= n log n-n+1+1/2 sum_(m = 1)^(n-1) 1/(m+1/2)
$
である。
$a >= 0$に対して
$
integral_1^(n-1) 1/(x+a) dd(x)
= eval(log(x+a))_1^(n-1)
= log(n-1+a)-log(1+a)
$
なので、
$
n log n-n+1+1/2 [log n-log 2+1/n]
<= log n!
<= n log n-n+1+1/2 [log(n-1/2)-log(3/2)+2/3]
$
が成り立つ。
ここから$log n!$の漸近近似として$n log n-n+1+1/2 log n$が得られて、
階乗に直すと$n!$と$e sqrt(n) (n/e)^n$でその比は
$
sqrt(1/2) exp(1/(2 n)) <= (n!)/(e sqrt(n) (n/e)^n) <= sqrt((n-1/2)/n) sqrt(2/3) exp(1/3)
$
を満たし、有界な範囲に留まるので、
$n!$はだいたい$e sqrt(n) (n/e)^n$と同じ速さで漸近近似される。
]
