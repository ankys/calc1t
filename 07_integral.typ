...
#block[
任意の分割$P$に対して、$inf_(x in [a, b]) f(x) (b-a) <= underline(S) (f, P) <= overline(S) (f, P) <= sup_(x in [a, b]) f(x) (b-a)$。
...
分割$P'$を$a = p'_0 <= p'_1 <= p'_2 <= p'_3 <= dots <= p'_M = b$として、\$\\lrset{ p\_0, p\_1, p\_2, p\_3, \\cdots, p\_N } \\subset \\lrset{ p\'\_0, p\'\_1, p\'\_2, p\'\_3, \\cdots, p\'\_M }\$となっている時、分割$P'$は分割$P$の_細分_という。
...
#block[
_Proof.]
まず、$overline(S) (f, P) <= sup_(x in [a, b]) f(x) (b-a)$,
...
今までに示した二つの命題より
$ underline(S) (f, underline(P)) <= underline(S) (f, P ') <= overline(S) (f, P ') <= overline(S) (f, overline(P)) $
が得られる。~◻
...
を導入すると以上の議論から $ underline(S) (f) <= overline(S) (f) $
を得る。

#block[
関数$f$が有界閉区間$[a, b]$上で_積分可能_あるいは_可積分_であるとは上記の式において等号が成り立つ場合をいい、その時の等しい値$underline(S) (f) = overline(S) (f)$を関数$f$の区間$[a, b]$上の_定積分_といい
$ integral_([a, b]) f(x) dd(x), quad integral_a^b f(x) dd(x) $
などで表す。 また$a < b$に対して、
$ integral_b^a f(x) dd(x) =-integral_a^b f(x) dd(x) $ と定義する。

]
#block[
1点からなる区間の時は $ integral_a^a f(x) dd(x) = 0 $
であることに注意する。

]
#block[
導入でも触れた通り定数関数$f(x) = c$は任意の有界閉区間で積分可能で
$ integral_a^b c dd(x) = c(b-a) $ が成り立つ。
...
分割の列$(P_n)$であって\$\\abs{P\_n} \\to 0\$かつ$overline(S) (f, P_n)-underline(S) (f, P_n) -> 0$を満たすものが存在することである。
...
$ overline(S) (f, P_n)-underline(S) (f, P_n) <= overline(S) (f, overline(P)_n)-underline(S) (f, underline(P)_n) -> overline(S) (f)-underline(S) (f) $
が成り立つ。
よって、$f$が積分可能であるときほしかった分割の列が得られた。

逆に$P_n$が存在するとき、
$ overline(S) (f)-underline(S) (f) <= overline(S) (f, P_n)-underline(S) (f, P_n) -> 0 $
...
#block[
_Proof.] 議論はほぼ同じなので、広義単調増加の場合のみ示す。
...
$ sup_(x in [p, q]) f(x)-inf_(x in [p, q]) f(x) = f(x^(‾))-f(y^(‾)) <= omega (lr(|x^(‾)-y^(‾)|)) <= omega (q-p) $
...
$ integral_a^b (f(x)+g (x)) dd(x) = integral_a^b f(x) dd(x)+integral_a^b f(x) dd(x), quad integral_a^b c f(x) dd(x) = c integral_a^b f(x) dd(x) $
が成り立つ。
...
$ overline(S) (f+g, P) = sum_(n = 1)^N sup_(x in [p_(n-1), p_n]) (f(x)+g (x)) (p_n-p_(n-1)) <= overline(S) (f, P)+overline(S) (g, P) . $
同様にして
$ underline(S) (f+g, P) = sum_(n = 1)^N inf_(x in [p_(n-1), p_n]) (f(x)+g (x)) (p_n-p_(n-1)) gt.eq underline(S) (f, P)+underline(S) (g, P) . $
したがって
$ underline(S) (f, P)+underline(S) (g, P) <= underline(S) (f+g, P) <= overline(S) (f+g, P) <= overline(S) (f, P)+overline(S) (g, P) $
...
$g$が各$x in [a, b]$に対して$f(x) <= g (x)$を満たすとき、
...
$f$も積分可能で
$ integral_a^b f(x) dd(x) = integral_a^b tilde(f) (x) dd(x) $ が成り立つ。
...
$ overline(S) (f) <= integral_a^c f(x) dd(x)+integral_c^b f(x) dd(x), quad underline(S) (f) gt.eq integral_a^c f(x) dd(x)+integral_c^b f(x) dd(x) $
となるからほしかった等式が得られる。~◻
...
$ integral lr(|x|)^a dd(x) = frac(1, a+1) x lr(|x|)^a+C quad (a eq.not-1, x eq.not 0) . $
$ integral e^x dd(x) = e^x+C, quad integral a^x dd(x) = frac(1, log a) a^x+C quad (a > 0, a eq.not 1) . $
$ integral 1/x dd(x) = log lr(|x|)+C quad (x eq.not 0), $
...
$ integral x^a dd(x) = cases(delim: "{", frac(1, a+1) x^(a+1)+C & upright("(") a eq.not-1 upright("),"), log lr(|x|)+C & upright("(") a =-1 upright(")"), ) $
と覚えておけばよい。
...
#block[
$ integral log lr(|x|) dd(x) = x log lr(|x|)-x+C quad (x eq.not 0) . $
$ integral tan x dd(x) =-log lr(|cos x|)+C quad (x eq.not pi/2+n pi, n in bb(Z)) . $
$ integral tanh x dd(x) = log (cosh x)+C . $

]
#block[
_Proof.] 証明は右辺を微分することで確かめられるので省略する。~◻
...
#block[
_Proof.] $y = f^(- 1) (x)$と置換すると、$x = f(y)$より
...
$ I_(n+1) <= I_n <= I_(n-1) $ であり、$n I_n gt.eq 0$をかけて
...
#block[
_Proof.] \$\$A = \\qty(\\int\_a^b |f(x)|^p\\dd{x})^{\\frac{1}{p}},
...
#block[
_Proof.] 途中で三角不等式を一回使うことで
$ integral_a^b lr(|f(x)+g (x)|)^p dd(x) & = integral_a^b lr(|f(x)+g (x)|) lr(|f(x)+g (x)|)^(p-1) dd(x)\
 & <= integral_a^b lr(|f(x)|) lr(|f(x)+g (x)|)^(p-1) dd(x)+integral_a^b lr(|g (x)|) lr(|f(x)+g (x)|)^(p-1) dd(x) $
を得る。
...
#block[
$a$を実数とするとき、
$ integral_1^oo x^a dd(x) = cases(delim: "{",+oo & (a gt.eq-1) ,, frac(1,-a-1) & (a <-1) .) $
$ integral_0^1 x^a dd(x) = cases(delim: "{", frac(1, a+1) & (a >-1) ,,+oo & (a <=-1) .) $

]
#block[
_Proof.] 前半は$a eq.not-1$のとき$t -> oo$で、
...
$ integral_I lr(|f(x)|) dd(x) $
が収束する時、$f$は$I$上で_絶対広義積分可能_または広義積分が_絶対収束_するという。

]
#block[
広義積分が絶対収束する場合 $ integral_I lr(|f(x)|) dd(x) < oo $ と表し、
絶対収束しない場合 $ integral_I lr(|f(x)|) dd(x) = oo $ と表す。
...
#block[
_Proof.] $I = \[ a, b \)$の場合のみ示す。 $t in I$に対して、
...
- 二つの条件
  $ lr(|f(x)|) <= g (x) quad forall x in I, quad integral_I g (x) < oo $
