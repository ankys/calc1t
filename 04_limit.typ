...
この時$N$以上のすべての自然数$n$に対して$n >= N > M$より、数列$(n)_(n = 0)^oo$は$n -> oo$で$+oo$に発散することがわかる。
]
...
特に$(a_n)$, $(b_n)$を、各自然数$n$に対して$c_n = (a_n+b_n)/2$として$a_(n+1) = a_n$, $b_(n+1) = c_n$または$a_(n+1) = c_n$, $b_(n+1) = b_n$が成立するように定義する時、$a_n -> x$, $b_n -> x$となる実数$x$が存在する。
]
