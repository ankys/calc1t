...
#block[
<t_squeeze> $(a_n)$, $(b_n)$を実数列とし、それぞれ実数$a$,
$b$に収束するとする。
...
$ c_n-a >= a_n-a >-epsilon, quad c_n-b <= b_n-b < epsilon $
でまとめると $ abs(c_n-a|) = abs(c_n-b|) < epsilon $
なので、$c_n$は$a = b$に収束する。
...
#block[
$0 <= abs(a_n-a|) <= b_n$であり、$(0)$,
$(b_n)$は$0$に収束することから、はさみうちの原理（命題@t_squeeze）より、実数列$(abs(a_n-a|))_n$は$0$に収束する。
...
+ \$\\max\\lrset{ a\_n, b\_n } \\to \\max\\lrset{ a, b }\$.

+ \$\\min\\lrset{ a\_n, b\_n } \\to \\min\\lrset{ a, b }\$.
...
+ 二つ目で$a_n$として$b_n$、$c$として$- 1$として適用すれば$- b_n ->-b$である。
  よって、一つ目より
  $ a_n-b_n = a_n+(- b_n) -> a+(- b) = a-b $ である。

+ 計算すると
  $ abs(a_n b_n-a b|) = abs(a_n (b_n-b)+(a_n-a) b|) <= abs(a_n|) abs(b_n-b|)+abs(b|) abs(a_n-a|) . $
  ここで、$(a_n)$は収束するので命題@t_conv_bddより有界であるからある実数$M$が存在してすべての自然数$n$に対して$abs(a_n-a|) <= M$とできることから、
  $ abs(a_n b_n-a b|) <= M abs(b_n-b|)+abs(b|) abs(a_n-a|) . $
  したがって一つ目と二つ目を使うことで右辺は$0$に収束することがわかる。
  よって、系@t_squeeze2より$a_n b_n -> a b$が結論付けられる。

+ まず、$b_n -> b eq.not 0$より極限の定義で$epsilon$として$abs(b|)/2 > 0$として取れば、十分大きな自然数$n$に対して$abs(b_n-b|) < abs(b|)/2$である。
  したがって三角不等式より、
  $ abs(b_n|) = abs((b_n-b)+b|) >= abs(|) b abs(-|) b_n-b abs(|) >= abs(b|)-abs(b_n-b|) > abs(b|)-abs(b|)/2 = abs(b|)/2 > 0 . $
  よって特に$b_n eq.not 0$である。 また、計算すると
  \$\$\\lrabs{\\frac{a\_n}{b\_n}-\\frac{a}{b}} = \\frac{|a\_n b-a b\_n|}{|b||b\_n|} = \\frac{|(a\_n-a)b-a(b\_n-b)|}{|b||b\_n|} \\le \\frac{|b||a\_n-a|+|a||b\_n-b|}{|b||b\_n|} \\le 2\\frac{|b||a\_n-a|+|a||b\_n-b|}{|b|^2}.\$\$
  ここで一つ目と二つ目を使うことで右辺は$0$に収束することがわかる。
  よって、系@t_squeeze2より$a_n/b_n -> a/b$が結論付けられる。

+ 三角不等式より、 $ abs(|) a_n abs(-|) a abs(|) <= abs(a_n-a|) $
  なので、系@t_squeeze2より$abs(a_n|) -> abs(a|)$がわかる。

+ 一般に実数$x$, $y$に対して
  \$\$\\max\\lrset{ x, y } = \\frac{x+y}{2}+\\frac{|y-x|}{2}\$\$
  が成り立つことに注意する。 よって
  \$\$\\max\\lrset{ a\_n, b\_n } = \\frac{a\_n+b\_n}{2}+\\frac{|b\_n-a\_n|}{2} \\to \\frac{a+b}{2}+\\frac{|b-a|}{2} = \\max\\lrset{ a, b }\$\$
  である。

+ 一般に実数$x$, $y$に対して
  \$\$\\min\\lrset{ x, y } = \\frac{x+y}{2}-\\frac{|y-x|}{2}\$\$
  が成り立つことに注意する。 よって
  \$\$\\min\\lrset{ a\_n, b\_n } = \\frac{a\_n+b\_n}{2}-\\frac{|b\_n-a\_n|}{2} \\to \\frac{a+b}{2}-\\frac{|b-a|}{2} = \\min\\lrset{ a, b }\$\$
  である。
...
+ 十分大きい任意の自然数$n$に対して$a_n > 0$かつ$a_n -> 0$ならば$a_n^(- 1) ->+oo$。
...
よって、$a_n^(- 1) -> 0$である。 二つ目は同様にして証明できる。
...
- $frac(n+2, 3 n+4) = frac(1+2 n^(- 1), 3+4 n^(- 1)) -> frac(1+2 dot.op 0, 3+4 dot.op 0) = 1/3$.
...
+ $c <=-1$の時、$(c^n)$は$n -> oo$で発散する（$plus.minus oo$に発散もしない）。
...
$0 <= c < 1$より$d = frac(1+c, 2)$とおくと$0 <= d < 1$で$d$を公比とする等比数列に帰着させたい。
そこで$epsilon = d-c = frac(1-c, 2) > 0$とおくと、十分大きいすべての自然数$n$に対して
\$\$\\lrabs{\\frac{a\_{n+1}}{a\_n}} \\le c+\\varepsilon = d\$\$
つまり、$abs(a_(n+1)|) <= d abs(a_n|)$である。
したがってこれを繰り返し用いることで、
$ abs(a_n|) <= d abs(a_(n-1)|) <= d^2 abs(a_(n-2)|) <= dots <= d^n abs(a_0|) . $
よって、$0 <= d < 1$より命題@t_limgeomseqから最右辺は$0$に収束するので、はさみうちの原理から$a_n -> 0$である。
