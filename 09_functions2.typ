...
== 複素指数関数
<複素指数関数>
指数関数はテーラー展開#link(<e_exp_power>)[\[e\_exp\_power\]];が知られているが、
べき級数の節で触れたようにべき級数の理論は複素数の範囲に自然と拡張できるので、
指数関数をべき級数によって定義することで定義される範囲を拡張することができる。
つまり、複素数$z$に対して
$ exp (z) = e^z = sum_(n = 0)^oo frac(1, n !) z^n = 1 + z + 1 / 2 z^2 + frac(1, 3 !) z^3 + dots.h.c $
とする。

このときべき級数を比較すると、オイラーの公式
$ e^(i theta) = cos theta + i sin theta , quad e^(- i theta) = cos theta - i sin theta $
が成立することがわかる。
...
$ lim_(x arrow.r 0) frac(sin x, x) = 1 $ がわかる。

指数法則を使えば指数関数や三角関数の和について以下がわかる。

#block[
複素数$x$に対して
$ sum_(n = M)^N e^(n x) = cases(delim: "{", frac(e^((N + 1) x) - e^(M x), e^x - 1) & (e^x eq.not 1) ,, N - M + 1 & (e^x = 1) ., ) $
$ sum_(n = M)^N sin n x = $
...
正確には実数全体で定義された関数 \$\$\\sinc x =
\\begin{cases}
\\frac{\\sin x}{x} & (x \\ne 0), \\\\
1 & (x = 0) \\\\
\\end{cases}\$\$ を#emph[シンク関数];という。

シンク関数は極限の式から$bb(R)$上の連続関数であることがわかるが、
べき級数での表示
\$\$\\sinc x = \\sum\_{k = 0}^\\infty \\frac{(-1)^k}{(2 k+1)!}x^{2 k} = 1-\\frac{1}{6}x^2+\\frac{1}{5!}x^4-\\cdots\$\$
が成り立つので、無限回微分可能な関数である。

シンク関数が登場した場面を思い出すと広義積分と級数の条件収束の例としてである。

ここでは広義積分\$\\int\_0^\\infty \\sinc x\\dd{x}\$が収束するが、絶対収束しないことを示そう。
まず\$\\int\_0^1 \\sinc x\$は普通の定積分であることに注意する。
$t > 1$に対して部分積分より
\$\$\\int\_1^t \\sinc x\\dd{x} = \\int\_1^t \\frac{(-\\cos x)\'}{x}\\dd{x} = \\lreval{\\frac{(-\\cos x)\'}{x}}\_1^t-\\int\_1^t \\frac{\\cos x}{x^2}\\dd{x}\$\$
となる。 ここで、 \$\$\\abs{\\frac{\\cos x}{x^2}} \\le \\frac{1}{x^2},
\\quad \\int\_1^\\infty \\frac{1}{x^2}\\dd{x} \< \\infty\$\$
なので、広義積分$integral_1^oo frac(cos x, x^2) ⅆ x$は収束し、広義積分\$\\int\_0^\\infty \\sinc x\\dd{x}\$も収束が言える。
一方で自然数$N = 0 , 1 , 2 , 3 , dots.h.c$に対して
\$\$\\int\_0^{2\\pi N} |\\sinc x|\\dd{x} = \\sum\_{n = 1}^N \\int\_{2\\pi(n-1)}^{2\\pi n} \\frac{|\\sin x|}{x}\\dd{x} \\ge \\sum\_{n = 1}^N \\int\_{2\\pi(n-1)}^{2\\pi n} \\frac{|\\sin x|}{2\\pi n}\\dd{x} = \\sum\_{n = 1}^N \\frac{2}{\\pi n}\$\$
であり、$N arrow.r oo$とすると右辺は正の無限大に発散するので、広義積分\$\\int\_0^\\infty \\sinc x\\dd{x}\$は絶対収束しない。

ちなみに広義積分\$\\int\_0^\\infty \\sinc x\\dd{x}\$の値は$pi / 2$であることが知られていて#emph[ディリクレ積分];と呼ばれるが、証明には複素解析学の知識が必要であり本テキストの範囲を逸脱するので取り扱わない。

== ガウス関数
<ガウス関数>
やはり広義積分の部分で登場した$x$について関数
$ exp (- x^2) = e^(- x^2) $ は#emph[ガウス関数];と呼ばれる。
...
ガウス関数はべき級数での表示
$ exp (- x^2) = sum_(n = 0)^oo frac(1, n !) (- x^2)^n = 1 - 1 / 2 x^2 + frac(1, 4 !) x^4 - frac(1, 6 !) x^6 + dots.h.c $
があり、収束半径は$oo$である。

一方で原始関数はこれまでに登場した関数で表すことができないことが知られている（ガウスの誤差関数と呼ばれる）。
しかしながら広義積分
$ integral_(- oo)^(+ oo) exp (- x^2) ⅆ x = integral_(- oo)^(+ oo) e^(- x^2) ⅆ x $
の値は計算できる。 これを#emph[ガウス積分];という。
...
この不等式を$n = 1 , 2 , 3 , dots.h.c$乗して積分すると次が得れれる。
...
である。
したがって、$n arrow.r oo$とするとウォリスの公式（定理@t_wallis_limit）よりガウス積分の値が
...
#block[
任意の$n = 0 , 1 , 2 , 3 , dots.h.c$に対して、 $ Gamma (n + 1) = n ! $
が成り立つ。
