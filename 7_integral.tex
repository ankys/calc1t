...
$$
\int_a^b f(x)\dd{x} \le \int_a^b f(x)\dd{x}
$$
が成り立つ。
...
実数係数の有理式つまり多項式$p(x) \ne 0$分の多項式$q(x)$の形$\frac{q(x)}{p(x)}$になっている関数も積分が計算できる。
...
\item[(iii)]
判別式が負な二次式の累乗で次の形$\frac{a(x-b)}{((x-b)^2+c^2)^n}$ ($a, b c \in \mathbb{R}$, $n = 1, 2, 3, \cdots$)。
\item[(iv)]
...
実際
$$
e^{-x^2} \le \min\lrset{ \frac{1}{x^2}, 1 }
$$
であることから従う。
\end{example}
